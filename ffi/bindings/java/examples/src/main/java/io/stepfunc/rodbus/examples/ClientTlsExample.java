...
public class ClientTlsExample {
    ...
    private static void runOneCommand(ClientChannel channel, String command) {
        ...
        switch (command) {
            case "rc": {
                // ANCHOR: read_coils
                channel.readCoils(param, range).whenComplete(ClientTlsExample::handleBitResult);
                // ANCHOR_END: read_coils
                break;
            }
            case "rdi": {
                channel.readDiscreteInputs(param, range).whenComplete(ClientTlsExample::handleBitResult);
                break;
            }
            case "rhr": {
                channel.readHoldingRegisters(param, range).whenComplete(ClientTlsExample::handleRegisterResult);
                break;
            }
            case "rir": {
                channel.readInputRegisters(param, range).whenComplete(ClientTlsExample::handleRegisterResult);
                break;
            }
            case "wsc": {
                /// ANCHOR: write_single_coil
                channel.writeSingleCoil(param, new BitValue(ushort(0), true)).whenComplete(ClientTlsExample::handleWriteResult);
                /// ANCHOR_END: write_single_coil
                break;
            }
            case "wsr": {
                channel.writeSingleRegister(param, new RegisterValue(ushort(0), ushort(76))).whenComplete(ClientTlsExample::handleWriteResult);
                break;
            }
            case "wmc": {
                channel.writeMultipleCoils(param, ushort(0), Arrays.asList(true, false)).whenComplete(ClientTlsExample::handleWriteResult);
                break;
            }
            case "wmr": {
                // ANCHOR: write_multiple_registers
                channel.writeMultipleRegisters(param, ushort(0), Arrays.asList(ushort(0xCA), ushort(0xFE))).whenComplete(ClientTlsExample::handleWriteResult);
