...
You can create a serial RTU client channel using `create_rtu_client` method. It will immediately try to open
the serial port.

:::note
In Rust, you can use the `spawn_rtu_client_task` to create a channel and spawn the runner task in the current runtime.
Otherwise, you can use `create_rtu_handle_and_task` and manually spawn the returned future when ready.
:::

:::info
This library currently does **not** support sending broadcast requests. Sending a request with a broadcast unit ID
