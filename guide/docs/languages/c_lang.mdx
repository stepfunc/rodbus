...
The C bindings feature a simple single-header design that simplifies application integration:
...
:::tip
The CMake script shows how to integrate the library; you don't have to use it. The distribution contains a single header file
and a shared library that link easily. Assuming that you've installed the shared library in /usr/lib, linking a program could be as simple
as:

```
> gcc main.c -I./c-bindings/include/ -lrodbus_ffi
> ldd a.out
        linux-vdso.so.1 (0x00007ffd643f1000)
        librodbus_ffi.so => /lib/librodbus_ffi.so (0x00007fbb1ec5c000)
        libc.so.6 => /lib/x86_64-linux-gnu/libc.so.6 (0x00007fbb1ea6a000)
        libgcc_s.so.1 => /lib/x86_64-linux-gnu/libgcc_s.so.1 (0x00007fbb1ea4f000)
        librt.so.1 => /lib/x86_64-linux-gnu/librt.so.1 (0x00007fbb1ea45000)
        libpthread.so.0 => /lib/x86_64-linux-gnu/libpthread.so.0 (0x00007fbb1ea22000)
        libm.so.6 => /lib/x86_64-linux-gnu/libm.so.6 (0x00007fbb1e8d3000)
        libdl.so.2 => /lib/x86_64-linux-gnu/libdl.so.2 (0x00007fbb1e8cb000)
        /lib64/ld-linux-x86-64.so.2 (0x00007fbb1f1cd000)
```
:::
...
> ldd a.out
        ...
        librodbus_ffi.so => /usr/lib/librodbus_ffi.so (0x7fbb1e894000)
...
- `rodbus` which is a shared library. When using this target, you must make the DLL or .so file available to your executable. You can automatically copy it to the executable target directory using the following CMake
  statement:
...
This example contains only a single method; other interfaces contain several methods.
...
The C bindings implement collections as opaque iterator types. Think of them as a class with a single "next" method. For example, let's look at an iterator over bool values:
...
:::warning
Never use an iterator outside its callback. Iterators typically reference stack memory, causing undefined behavior if used after callback completion.
:::

### Error Handling

The C API uses error codes for error handling. Error codes are enum values where the first value equals zero, indicating success.
