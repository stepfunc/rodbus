...
We distribute the C# bindings as a [Nuget package](https://www.nuget.org/packages/rodbus). Internally, the C API is called using [P/Invoke](https://docs.microsoft.com/en-us/dotnet/standard/native-interop/pinvoke)
...
:::warning
Uncaught exceptions thrown in callbacks will terminate the program. Your code should always wrap callback logic with `try/catch` syntax if there is a chance that the callback will throw.
:::
